...
(deftest states
  ...
  (testing "F7 Available"
    (let [res (resource {"text/html" (fn [r] {:body (:acceptable-encoding r)})})
          req1 (assoc-in test-request [:headers "accept-encoding"]
                         "gzip,*;q=0.8")
          req2 (assoc-in test-request [:headers "accept-encoding"]
                         "gzip, deflate")
          response1 (run req1 res)
          response2 (run req2 res)]
      (is (= 200 (:status response1)))
      (is (= 200 (:status response2)))))

  (testing "F7 Not Available"
    (let [res (resource {"text/html" "testing"})
  ...
  #_  (testing "G7 Header"
        (let [res (resource {"text/html" "testing"})
          ...
          (let [response (run req res)]
            ...
            (is (= "accept-encoding, accept-charset, accept-language, accept"
